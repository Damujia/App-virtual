<template>
  <div class="detail">
    <van-nav-bar :fixed="true" :placeholder="true" z-index="99" title="生产记录" left-text="返回" left-arrow @click-left="onClickLeft" />
      ...
      <div class="detail_bod_one">
        ...
        <div v-for="(val, index) in listData" :key="index" class="detail_row">
          <van-row class="row_tit">
            ...
            <van-col @click="getorderMore(val)" span="4">更多<van-icon name="arrow" /></van-col>
          </van-row>
          ...
          <van-row type="flex" justify="space-between">
            ...
            <van-col><span class="col_font_text">{{val.workerNo}}</span></van-col>
          </van-row>
          <van-row type="flex" justify="space-between">
            <van-col><span class="col_font">传感器：</span></van-col>
            <van-col><span class="col_font_text">{{val.sensorNo}}</span></van-col>
          </van-row>
          <van-row type="flex" justify="space-between">
            <van-col><span class="col_font">创建时间：</span></van-col>
            <van-col><span class="col_font_text">{{val.createTime}}</span></van-col>
          </van-row>
        </div>
      </div>
    </van-pull-refresh>
...
export default {
  ...
  methods: {
    getorderMore(val){
      this.$router.push({name: 'ordermore'})
      this.$store.commit('setdetail', val)
    },
    onClickLeft () {
      this.$router.push({ name: 'orderinfo' })
    },
    async getinit (type) {
      let res = await this.$api.getProManageProRecordMenu(this.getobj)
