<template>
  <div class="detail">
    <van-nav-bar :fixed="true" :placeholder="true" z-index="99" title="出库单-详情" left-text="返回" left-arrow @click-left="onClickLeft" />
      ...
      <div class="detail_bod_one">
        ...
        <div v-for="(val, index) in listData" :key="index" class="detail_row">
          <van-row class="row_tit">
            ...
            <!-- <van-col @click="btnbadurl(val)" span="4"><van-icon name="arrow" /></van-col> -->
          </van-row>
          ...
          <van-row type="flex" justify="space-between">
            ...
            <van-col><span class="col_font_text">{{val.putOutTime}}</span></van-col>
          </van-row>
          <van-row type="flex" justify="space-between">
...
export default {
  ...
  mounted () {
    ...
    this.getobj.putOutTimeStamp = orderObj
    this.getinit()
  },
  methods: {
    onClickLeft () {
    ...
    async getinit (type) {
      let res = await this.$api.getStoragePutOutDetail(this.getobj)
