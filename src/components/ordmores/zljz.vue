    ...
    <van-pull-refresh style="width: 100%;overflow-y: auto;" v-model="isLoading" @refresh="getinfo(true)">
      <div class="detail_stock_one">
        ...
        <div v-for="(val, index) in datas" :key="index" class="detail_row">
          ...
          <van-row type="flex" justify="space-between">
            <van-col><span class="col_font">重量：</span></van-col>
            <van-col><span class="col_font_text">{{val.zl}}</span></van-col>
          </van-row>
          <van-row type="flex" justify="space-between">
            <van-col><span class="col_font">G值：</span></van-col>
            <van-col><span class="col_font_text">{{val.extremumNumber}}</span></van-col>
          </van-row>
          <van-row type="flex" justify="space-between">
...
export default {
  ...
  methods: {
    async getinfo (type) {
      let res = await this.$api.getDataZzjz(this.getobj)
